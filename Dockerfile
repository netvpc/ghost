...
RUN apt-get update \
    && apt-get -y --no-install-recommends install tini git ca-certificates \
    && rm -rf /var/lib/apt/lists/* \
    && apt-get clean

COPY ghost-5.88.2.tgz /ghost-5.88.2.tgz
...
RUN set -eux; \
	...
	aptPurge=; \
	\
	installCmd='gosu node ghost install --db mysql --dbhost mysql --no-prompt --no-stack --no-setup --dir "$GHOST_INSTALL" --archive "/ghost-5.88.2.tgz"'; \
